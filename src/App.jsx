...
function App() {
  ...
  const handleStatusChange = (leadId, newStatusColumnId, currentColumnId) => {
    ...
    setLeads(prevLeads => {
      ...
      // Garante que a coluna de destino é um array, mesmo que vazia
      const updatedTargetLeads = [...prevLeads[newStatusColumnId], updatedLead];
      ...
      <div className="relative inline-block text-left z-10">
        <button
          ...
          className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-semibold transition-colors
            ${currentStatus.color} text-white hover:opacity-90`}
        >
          {currentStatus.label}
